...
describe("conventional commits", () => {
  ...
  it("categorizes refactor: prefix", () =>
    expect(categorize("refactor: extract helper")).toBe("refactor"));
  it("categorizes chore: prefix as refactor", () =>
    expect(categorize("chore: bump deps")).toBe("refactor"));
  ...
  it("handles scope — feat(auth): message", () =>
    expect(categorize("feat(auth): add OAuth")).toBe("feat"));
  it("handles scope — fix(api): message", () =>
    expect(categorize("fix(api): timeout error")).toBe("fix"));
  it("handles breaking change — feat!: message", () =>
    expect(categorize("feat!: drop Node 14")).toBe("feat"));
  it("handles breaking change with scope — fix(core)!: message", () =>
    expect(categorize("fix(core)!: remove deprecated method")).toBe("fix"));
});
...
describe("userKeywords override", () => {
  ...
  it("matches user keyword", () =>
    expect(categorize("update terraform config", keywords)).toBe("infra"));
  it("matches user keyword case-insensitively", () =>
    expect(categorize("K8S rollout fix", keywords)).toBe("infra"));
