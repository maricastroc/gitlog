    ...
    >
      <Popover.Trigger className="flex items-center justify-between w-full px-4 py-3 rounded-lg bg-panel-2 border border-line text-text font-mono text-[13px] cursor-pointer outline-none gap-2 hover:border-text-dim transition-colors">
        <span className="truncate">
          {selected ? selected.label : <span className="text-text-dim">{placeholder}</span>}
        </span>
      ...
      <Popover.Portal>
        <Popover.Content
          sideOffset={4}
          className="z-50 w-[var(--radix-popover-trigger-width)] bg-panel-2 border border-line rounded-lg shadow-[0_8px_24px_rgba(0,0,0,0.5)] overflow-hidden"
        >
          ...
          <div className="p-1 max-h-64 overflow-y-auto scrollbar-thin scrollbar-thumb-line scrollbar-track-transparent">
            ...
            {filtered.length === 0 && (
              <p className="px-3 py-4 text-[12px] font-mono text-text-dim/50 text-center">
                No results
              </p>
            )}
          </div>
